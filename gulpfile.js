(function() {
    ...
    var USER_LIBRARY_NAME = 'scan';
    var SCAN_LIBRARY_NAME = USER_LIBRARY_NAME + '.js';
    var SCAN_LIBRARY_NAME_MIN = SCAN_LIBRARY_NAME.replace('.js', '.min.js');
    var BROWSERIFIED_LIBRARY_NAME = SCAN_LIBRARY_NAME.replace('.js', '.browserified.js');
    ...
    gulp.task('build', ['jshint', 'browserify'], function() {
        return gulp.src(BROWSERIFIED_JS_FILES)
            .pipe(gulp.dest(DEST_DIR))
            .pipe(uglify())
            .pipe(rename(SCAN_LIBRARY_NAME_MIN))
            .pipe(gulp.dest(DEST_DIR))
            .pipe(filesize())
            .on('error', gutil.log);
    });
